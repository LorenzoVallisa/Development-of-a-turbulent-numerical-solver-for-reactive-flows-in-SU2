...
//
//
CReactiveEulerSolver::CReactiveEulerSolver():CSolver(),nSpecies(),nMarker(),space_centered(),implicit(),least_squares(),Gamma(),Gamma_Minus_One(),
...
CReactiveEulerSolver::CReactiveEulerSolver(std::unique_ptr<CGeometry>& geometry, std::unique_ptr<CConfig>& config,unsigned short iMesh):
  ...
  nPrimVarGrad = nDim + 3; /*--- Gradient Primitive variables (T,vx,vy,vz,P,rho)^T ---*/
 ...
 */
 void CReactiveEulerSolver::SetNondimensionalization(CGeometry* geometry, CConfig* config, unsigned short iMesh) {
   ...
   /*su2double Temperature_FreeStream = 0.0, ModVel_FreeStream = 0.0, Energy_FreeStream = 0.0,
             ModVel_FreeStreamND = 0.0, Viscosity_FreeStream = 0.0, Density_FreeStream = 0.0,
             Pressure_FreeStream = 0.0, Mach2Vel_Freestream = 0.0;*/
   su2double Length_Ref = 0.0, Density_Ref = 0.0, Pressure_Ref = 0.0, Velocity_Ref = 0.0,
             Temperature_Ref = 0.0, Time_Ref = 0.0, Gas_Constant_Ref = 0.0, Viscosity_Ref = 0.0,
             Conductivity_Ref = 0.0, Energy_Ref = 0.0;
   su2double Pressure_FreeStreamND = 0.0, Density_FreeStreamND = 0.0,
             ...
             Energy_FreeStreamND = 0.0, Total_UnstTimeND = 0.0, Delta_UnstTimeND = 0.0, TgammaR = 0.0;
  ...
  */
   ...
   */
   ...
   if (config->GetConsole_Output_Verb() == VERB_HIGH and rank == MASTER_NODE and iMesh == MESH_0) {
     ...
     std::cout << "Reference specific gas constant: " << config->GetGas_Constant_Ref();
     if (config->GetSystemMeasurements() == SI)
       std::cout << " N.m/kg.K." << std::endl;
     else if (config->GetSystemMeasurements() == US)
       std::cout << " lbf.ft/slug.R." << std::endl;

     std::cout << "Reference pressure: " << config->GetPressure_Ref();
     if (config->GetSystemMeasurements() == SI)
       std::cout << " Pa." << std::endl;
     else if (config->GetSystemMeasurements() == US)
       std::cout << " psf." << std::endl;

     std::cout << "Reference temperature: " << config->GetTemperature_Ref();
     if (config->GetSystemMeasurements() == SI)
       std::cout << " K." << std::endl;
     else if (config->GetSystemMeasurements() == US)
       std::cout << " R." << std::endl;

     std::cout << "Reference density: " << config->GetDensity_Ref();
     if (config->GetSystemMeasurements() == SI)
       std::cout << " kg/m^3." << std::endl;
     else if (config->GetSystemMeasurements() == US)
       std::cout << " slug/ft^3." << std::endl;

     std::cout << "Reference velocity: " << config->GetVelocity_Ref();
     if (config->GetSystemMeasurements() == SI)
       std::cout << " m/s." << std::endl;
     else if (config->GetSystemMeasurements() == US)
       std::cout << " ft/s." << std::endl;

     std::cout << "Reference energy per unit mass: " << config->GetEnergy_Ref();
     if (config->GetSystemMeasurements() == SI)
     ...
     if (unsteady)
       std::cout << "Reference time: " << config->GetTime_Ref() <<" s." << std::endl;
     ...
     std::cout << "Specific gas constant (non-dim): " << config->GetGas_ConstantND() << std::endl;
     ...
     if (nDim == 2) {
       std::cout << "Free-stream velocity (non-dim): (" << config->GetVelocity_FreeStreamND()[0] << ", ";
       std::cout << config->GetVelocity_FreeStreamND()[1] << "). ";
     }
     else {
       std::cout << "Free-stream velocity (non-dim): (" << config->GetVelocity_FreeStreamND()[0] << ", ";
       std::cout << config->GetVelocity_FreeStreamND()[1] << ", " << config->GetVelocity_FreeStreamND()[2] << "). ";
     }
     std::cout << "Magnitude: "    << config->GetModVel_FreeStreamND() << std::endl;
