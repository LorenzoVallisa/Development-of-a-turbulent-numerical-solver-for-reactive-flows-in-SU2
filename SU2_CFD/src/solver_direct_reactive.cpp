...
void CReactiveEulerSolver::Check_FreeStream_Solution(CConfig* config) {
  ...
  if(tkeNeeded)
    nonPhys_infty = node_infty->SetPrimVar(config,val_ke);
  else
    nonPhys_infty = node_infty->SetPrimVar(config);
...
void CReactiveNSSolver::SetNondimensionalization(CGeometry* geometry, CConfig* config, unsigned short iMesh) {
  ...
  if (tkeNeeded){
    ...
    su2double Gas_Constant_Ref = Energy_Ref/config->GetTemperature_Ref();
    config->SetGas_Constant_Ref(Gas_Constant_Ref);
...
void CReactiveNSSolver::Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config, unsigned short iMesh,
  ...
  /*--- Gradient computation first---*/
  if(least_squares)
    SetPrimitive_Gradient_LS(geometry, config);
  else if(config->GetKind_Gradient_Method() == GREEN_GAUSS)
    SetPrimitive_Gradient_GG(geometry, config);

  /*--- Vorticity and strainmag computation only afterwards---*/
  for (iPoint = 0; iPoint < nPoint; iPoint++) {
...
void CReactiveNSSolver::SetTime_Step(CGeometry* geometry, CSolver** solver_container, CConfig* config,
  ...
  for(iEdge = 0; iEdge < geometry->GetnEdge(); ++iEdge) {
    ...
    if(rans){
      ...
      Gamma =node[iPoint]->GetdPdU()[RHOE_INDEX_SOL]+1;
...
void CReactiveNSSolver::Viscous_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
  ...
  for(iEdge = 0; iEdge < geometry->GetnEdge(); ++iEdge) {
    ...
    if (config->GetKind_Turb_Model() == SST){
      ...
      numerics->Set_GradTKE(solver_container[TURB_SOL]->node[iPoint]->GetGradient()[0],
                            solver_container[TURB_SOL]->node[jPoint]->GetGradient()[0]);
    }
