...
void CMultiGridIntegration::MultiGrid_Iteration(CGeometry ***geometry,
  ...
  const bool startup_multigrid = ((config[iZone]->GetRestart_Flow())     &&
                                  (RunTime_EqSystem == RUNTIME_FLOW_SYS || RunTime_EqSystem == RUNTIME_REACTIVE_SYS) &&
                                  (Iteration == 0));
    ...
    case RUNTIME_REACTIVE_SYS:
      ...
      if (config->GetConvCriteria() == RESIDUAL) {
        if (config->GetResidual_Func_Flow() == RHO_RESIDUAL)
          (*monitor) = std::log10(solver_container[FinestMesh][FLOW_SOL]->GetRes_RMS(CReactiveEulerVariable::GetRHO_INDEX_SOL()));
        else if (config->GetResidual_Func_Flow() == RHO_ENERGY_RESIDUAL)
          (*monitor) = std::log10(solver_container[FinestMesh][FLOW_SOL]->GetRes_RMS(CReactiveEulerVariable::GetRHOE_INDEX_SOL()));
      }
