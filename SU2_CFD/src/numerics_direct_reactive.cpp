...
void CUpwReactiveAUSM::ComputeResidual(su2double* val_residual, su2double** val_Jacobian_i,
  ...
  RealVec Velocity_i(V_i + CReactiveEulerVariable::VX_INDEX_PRIM, V_i + CReactiveEulerVariable::VX_INDEX_PRIM + nDim);
	sq_vel_i = std::inner_product(Velocity_i.cbegin(),Velocity_i.cend(),Velocity_i.cbegin(),0.0);
  ...
  RealVec Velocity_j(V_j + CReactiveEulerVariable::VX_INDEX_PRIM, V_j + CReactiveEulerVariable::VX_INDEX_PRIM + nDim);
  sq_vel_j = std::inner_product(Velocity_j.cbegin(),Velocity_j.cend(),Velocity_j.cbegin(),0.0);
  ...
  ProjVelocity_i = std::inner_product(Velocity_i.cbegin(),Velocity_i.cend(),UnitNormal,0.0);
  ProjVelocity_i = std::inner_product(Velocity_j.cbegin(),Velocity_j.cend(),UnitNormal,0.0);
  ...
  for(iDim = 0; iDim < nDim; ++iDim) {
    Ys_i[CReactiveEulerVariable::RHOVX_INDEX_SOL + iDim] = Density_i*Velocity_i[iDim];
    Ys_j[CReactiveEulerVariable::RHOVX_INDEX_SOL + iDim] = Density_j*Velocity_j[iDim];
  }
...
CAvgGradReactive_Flow::CAvgGradReactive_Flow(unsigned short val_nDim, unsigned short val_nVar, std::shared_ptr<CConfig> config):
  ...
  Mean_PrimVar.resize(nDim+2);
  PrimVar_i.resize(nDim+2);
  PrimVar_j.resize(nDim+2);
  //Mean_GradPrimVar.resize(nDim+2,RealVec(nDim));
  Mean_GradPrimVar.resize(nDim + 2,nDim);
...
void CAvgGradReactive_Flow::GetViscousProjFlux(RealVec& val_primvar, RealMatrix& val_grad_primvar,
  ...
  for (iDim = 0; iDim < nDim; ++iDim) {
    ...
    /*--- Density contribution ---*/
    Flux_Tensor[CReactiveEulerVariable::RHO_INDEX_SOL][iDim] = 0.0;

    /*--- Shear stress related terms ---*/
    Flux_Tensor[CReactiveEulerVariable::RHOE_INDEX_SOL][iDim] = 0.0;
    for (jDim = 0; jDim < nDim; ++jDim) {
      Flux_Tensor[CReactiveEulerVariable::RHOVX_INDEX_SOL + jDim][iDim]  = tau[iDim][jDim];
      Flux_Tensor[CReactiveEulerVariable::RHOE_INDEX_SOL][iDim] += tau[iDim][jDim]*V[CReactiveEulerVariable::VX_INDEX_PRIM+jDim];
    }
...
void CAvgGradReactive_Flow::ComputeResidual(su2double* val_residual, su2double** val_Jacobian_i,
  ...
  // Mass fraction
  for (iSpecies = 0; iSpecies < nSpecies; ++iSpecies) {
    PrimVar_i[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] = V_i[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] /
                                                                  V_i[CReactiveEulerVariable::RHO_INDEX_PRIM];
    PrimVar_j[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] = V_j[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] /
                                                                  V_j[CReactiveEulerVariable::RHO_INDEX_PRIM];
    Mean_PrimVar[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] = 0.5*(PrimVar_i[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies] +
                                                                          PrimVar_j[CReactiveEulerVariable::RHOS_INDEX_PRIM+iSpecies]);
    for (iDim = 0; iDim < nDim; iDim++) {
  ...
  std::copy(Proj_Flux_Tensor,Proj_Flux_Tensor + nVar,val_residual);
    ...
    su2double dist_ij_2 = std::inner_product(Edge_Vector.cbegin(),Edge_Vector.cend(),Edge_Vector.cbegin(),0.0);
...
void CSourceReactive::ComputeChemistry(su2double* val_residual, su2double** val_Jacobian_i, CConfig* config) {
  ...
  //library->GetSourceTerm(......);
  residual *= Volume; //L'UNICA COSA A CUI STARE ATTENTI SONO I SEGNI PERCHE' QUESTO E' UN RESIDUO QUINDI MI SA CHE DEVO INVERTIRLI
  val_residual = residual.data();
  ...
  if (implicit) {
    ...
    throw Common::NotImplemented("Implicit method for source chemistry residual not yet implemented");
  }
